...
public class MonteCarloTreeSearchExplorationPipeline {
    ...
    public static void main(String[] args) {
        ...
        PCollection<GameStateNode.GameStateNodeInfo> stateNodes =
                pipeline
    ...
    private static Example encodeExperimentalFeatures(final GameStateNode.GameStateNodeInfo stateNode) {
        StateEncoder stateEncoder = new DefaultStateEncoder();

        return stateNode.toTFExample(stateEncoder, false);
    }

    private static Example encodeDirectFeatures(final GameStateNode.GameStateNodeInfo stateNode) {
        StateEncoder stateEncoder = new DefaultStateEncoder();
    ...
    public static class ExpandFn extends DoFn<Game.StateProto, GameStateNode.GameStateNodeInfo> {

        private final Counter counter = Metrics.counter(ExpandFn.class, "mcts-nodes-added");

        @ProcessElement
        public void process(
                ProcessContext context, @Element Game.StateProto root, OutputReceiver<GameStateNode.GameStateNodeInfo> out) {
            ...
            FastMonteCarloTreeSearch monteCarloTreeSearch =
                    new FastMonteCarloTreeSearch(GameSimulator.MINIMAX_TF, new GameStateNode(new State(root)));
            ...
            List<GameStateNode.GameStateNodeInfo> nodes = monteCarloTreeSearch.traverse();
            counter.inc(nodes.size());
            nodes.forEach(out::output);
        }
    }

    public static class EnrichFn extends DoFn<GameStateNode.GameStateNodeInfo, GameStateNode.GameStateNodeInfo> {
        ...
        @ProcessElement
        public void process(
                ProcessContext context, @Element GameStateNode.GameStateNodeInfo in, OutputReceiver<GameStateNode.GameStateNodeInfo> out) {
            observed.inc();
            ...
            GameStateNode.GameStateNodeInfo enriched = new GameStateNode.GameStateNodeInfo(in);
